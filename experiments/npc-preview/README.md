...
open-weight chat model could narrate two NPCs (Lulu, Zaz) *without* ever
...
- In `lulu-preview.html`, the bloom is granted by
...
npc-preview/
├── README.md                  (this file)
├── lulu-preview.html          playable preview, Rungs 1-3, debug panel
├── zaz-lab.html               debug/test-only contrast lab (Lulu vs Zaz)
├── npc/
...
│   ├── personas.js            Lulu + Zaz constitutions and curated fallback lines
...
```
 player action (click/tap)
        │
        ▼
 lulu-preview.html "engine" (deterministic, plain JS state machine)
 ...
 npc-gateway.js  ── parse JSON → response-schema.js validate()
        ...
        │
        ▼
 lulu-preview.html renders speech bubble + gesture (presentation ONLY),
...
`lulu-preview.html` has a gate selector at the top. Each gate proves one
...
- **Gate 0 — Connectivity** (the default, run this first). One button:
  Warren → localhost gateway → local model → **valid JSON** → one Lulu
...
- **Gate 1 — Lulu continuity** (`"same being twice"`). First Bloom
...
- **Gate 2 — Zaz differentiation** (`zaz-lab.html`, debug-only, unlocked
  after Gate 1). Sends the *identical* stimulus to Lulu and Zaz and
  **actively rejects** a collapse: `assessDivergence()` flags — with a
  visible FAIL — when the two voices are too alike (identical speech, or
  ≥80% word overlap with identical emotion). This is not a passive
  side-by-side. Success: `"two minds, not two skins."` Zaz never appears
  in the normal Rungs 1-3 progression (`personas.js`:
  `ZAZ.rung_available_from = null`). (Tests **G**, **G2**.)
...
Then in `lulu-preview.html` (or `zaz-lab.html`):
...
Open `npc-preview/lulu-preview.html` directly in a browser (double-click
...
to Lulu and Zaz and have `assessDivergence()` actively flag any collapse
...
suite is **14 tests**: the spec's A–J plus **G0** (Gate 0 connectivity),
**G2** (Gate 2 divergence actively flags a collapse), **C2** (existence
gate refuses a memory for a creation the engine never made), and **PV**
(provenance present and correctly typed for all three sources).

## The smallest valuable slice

If you only build one thing from this preview: **Lulu live dialogue +
one remembered bloom + one autonomous curiosity beat.** That's rungs
1–3 in `lulu-preview.html` end to end — discover Lulu, offer the seed,
watch the bloom open (deterministically), come back later and hear Lulu
reference it from memory, and once, unprompted, hear her wonder something
small aloud. Everything else in this preview (Zaz, the debug panel, the
...
After two sessions with Lulu, the tester answers four questions:

1. Did Lulu **notice** what you did?
2. Did Lulu **remember** something real?
3. Did Lulu **react differently** because of that memory?
...
| 1 | One live Lulu response | live run (mock-proven headless) |
...
| 5 | Zaz contrast lab (Gate 2 divergence, actively flagged) | headless + live |
...
### Illustrative Lulu transcript (Rungs 1→3)

```
[Gate 0] player: connectivity ping
Lulu (source: model, provenance {source:model, model:"gemma3:4b", fallback:false, latency_ms:~340, reason:null}):
  speech: "Oh! A little seed. Will you stay and watch it with me?"
  emotion: delighted   gesture: look_up   initiative: ask_question
...
Lulu (source: model, ~410ms):
  speech: "It hums like it remembers something. Did you feel that too?"
  emotion: curious   gesture: look_up   initiative: ask_question
...
Lulu (source: model, ~380ms):
  speech: "I wonder what color it dreams of becoming."
  emotion: warm   gesture: look_up

[Gate 1 · Next visit, simulated]
Lulu (source: memory_derived, provenance {source:memory_derived, model:null, fallback:false, ...}):
  speech: "You came back. I kept our flower."
...
Lulu: "It hums like it remembers something. Did you feel that too?"
...
player sees: curated fallback, e.g. "My thoughts are sleepy. Stay beside the flower a moment."
...
- Real Lulu and Zaz transcripts — the ones above are illustrative,
  generated by the deterministic mock, and are clearly labeled as such.
- Full behavioral proof of tests **G / G2** (Lulu vs Zaz distinctness) —
  this harness proves the two personas are *structurally* distinct
...
- The **Phase 0 live half** — the headless **G0** proves the gateway
  yields valid JSON and one Lulu line with a reachable adapter; only a
...
Everything else in the "Node tests" section above (the 14 tests
A–J plus G0, G2, C2, PV) is mechanically proven right now, in this repo,
with no live model, and re-runs identically on any machine with Node
installed.
