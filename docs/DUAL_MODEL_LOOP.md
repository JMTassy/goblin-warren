...
**Receipts:**
- **ADMITTED** (first true admission): sovereign ledger one-liner, fork (a), by explicit operator instruction "admit the sovereign ledger one-liner". Provenance: goblin seed S5 ("you won, you don't remember when") → deterministic proof (effects outlive receipts) → HAL-&-HER debate → referee bloat test (576KB @ 10k events) → convergence candidate → operator hand. Implemented: v2 logEvent cap removed; 4 sovereign assertions added (v2-selftest 61); all suites green (29/61/24); original violation proof now returns false. V0 canon keeps its cap (frozen, waived). Replay still NOT claimed — sovereign ledger is necessary, not sufficient, for Tier 2.
