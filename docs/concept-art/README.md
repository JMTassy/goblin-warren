...
| `vision-05-root-hollow.jpeg` | **Answers PARKED_CHIDDUSHIM.md #6** ("Background 3: The Root Hollow at Dusk"). Ancient glowing tree, root-hollow cottages, bioluminescent mushrooms, lanterns, a stone bridge over a glowing stream. Closest thing yet to a ready `#world` background candidate — not wired in; swapping/adding to the live background layer is its own decision, not a silent CSS drop. |
| `vision-06-wul-goblin.jpeg` | Mascot/vision-board illustration — a goblin child holding a "WUL" mug in a mushroom grove. Character mood reference, same tier as `lulu-tamagotchi-vision.jpeg`; polish level (semi-realistic children's-book render) is well above the game's current painterly-glow art direction, so treat as *mood*, not a production sprite source. |
| `vision-07-boss-realm-minigames.jpeg` | **A full design document, not art** — "The Abyssal Depths: Boss Realm" pitch: a *third* boss/zone concept (Cinder Wraith Lair, Magma Golem Forge, Void Portal — dragon/fire/void aesthetic, distinct from the four Temple Bosses) plus general minigame design philosophy (10 design-DNA principles, a fun-tuning formula, example minigame loops including "Matcha Brew"). Logged as PARKED_CHIDDUSHIM.md #16 — flagging for the operator to reconcile against the existing Temple Boss set rather than silently piling on a third boss concept. |
